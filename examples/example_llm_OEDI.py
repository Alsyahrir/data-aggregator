...
def main():
    ...
    df_aggregated.to_csv("outputs/OEDI/daily_aggregated.csv", index=False)
    print(f"\nSaved: outputs/OEDI/daily_aggregated.csv ({len(df_aggregated)} rows)")

    df_merged.to_csv("outputs/OEDI/standardized_raw.csv", index=False)
    print(f"Saved: outputs/OEDI/standardized_raw.csv ({len(df_merged)} rows)")
